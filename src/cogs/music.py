...
class Music(commands.Cog):
    def __init__(self, bot):
        ...
        if not hasattr(
            ...
            bot.lavalink.add_node(
                "lava.link", 80, "maybeiwasboring", "us", "gigabeats"
    ...
    @commands.command(
        name="play",
        description="Play a song/Unpause the player",
        aliases=["p"],
        usage="play <query>",
    )
    async def play(self, ctx, *, query: str = None):
        ...
        if results["loadType"] == "PLAYLIST_LOADED":
            ...
            embed.add_field(
                name="Duration",
                value=lavalink.format_time(int(track["info"]["length"])),
                inline=True,
            )
    ...
    @commands.command(
        name="pause",
        description="Pauses playback of the current playing song.",
        usage="pause",
    )
    ...
    @radio.command(
        name="RDMIX",
        description="Switches to RDMIX Radio",
        usage="radio rdmix",
        aliases=["rap", "hiphop", "rdmix"],
    )
    @has_voted()
    async def rdmix(self, ctx):
        ...
        if player.is_playing:
            if not ctx.author.guild_permissions.manage_guild:
                await music.play(
                    ctx, query="http://s2.ssl-stream.com/radio/8230/radio.mp3"
                )
            else:
                player.queue.clear()
                await player.stop()
    ...
    @radio.command(
        name="Chill",
        description="Switches to Chillstream Radio",
        usage="radio chill",
        aliases=["lofi", "chill", "chillout", "chillstream"],
    )
    @has_voted()
    ...
    async def queue(self, ctx):
        ...
        try:
            ...
            current = await player.node.get_tracks(
                f"ytsearch:{player.current.identifier}"
            )
            current = current["tracks"][0]
            emlist = []
    ...
    @commands.command(
        ...
        usage="clear",
        aliases=["clearqueue", "clearq", "clearque"],
    )
