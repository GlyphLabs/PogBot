...
class economy(commands.Cog, name="economy"):
    ...
    async def rob(self, ctx, *, member: discord.Member):
        ...
        if collection.count_documents({"_id": ctx.author.id}) == 0:
            await ctx.send(
                f"{member.name} isn't participating in our economy system yet! If they'd like to, ask them to type `pog bal`"
            )
            return
        else:
            ...
            if choice == 1:
                ...
                await ctx.send(
                    f"**{ctx.author.name}** just stole **{amt}** pog coins from **{member.name}**!"
                )
            else:
                wamt = amt - (amt * 2)
                ...
                await ctx.send(
                    f"**{ctx.author.name}** just tried to rob **{member.name}**, but was caught! They paid a **{amt}** pog coin fine."
                )
